    ...
    .show(ui, |ui| {
        ...
        if let Some(tex) = texture {
            ...
            // 底层水平渐变（饱和度）
            {
                ...
                let idx = mesh.vertices.len() as u32;
                mesh.colored_vertex(top_left, col_left); // 顶部顶点使用左边颜色
                mesh.colored_vertex(bottom_left, col_left);
                mesh.colored_vertex(middle_right, col_right);
            ...
            // 顶层垂直黑色渐变（明度）
            {
                ...
                let idx = mesh.vertices.len() as u32;
                mesh.colored_vertex(top_left, col_top);
                mesh.colored_vertex(middle_right, col_top);
                mesh.colored_vertex(bottom_left, col_bottom);
            ...
            let sl_y = 
            triangle_rect.top() + triangle_size * (1.0 - hsl.l);
            painter.circle_stroke(Pos2::new(sl_x, sl_y), 4.0, Stroke::new(2.0, color_revert));
        }
    });
}
